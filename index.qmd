...
| U.S. Department of Energy Alternative Fuels Data Center, Electric Vehicle Registrations by State | 2023 light-duty EV registration counts |
| U.S. Census Bureau Population Estimates Program | 2023 resident population and Census region |
| U.S. Census Bureau Small Area Income and Poverty Estimates | 2023 median household income and all-ages poverty rate |
...
![Bayesian network used for state-level EV charging access.](images/pgm_dag.svg){#fig-dag width="80%"}
...
where $\alpha=1$. Smoothing is important here because the dataset is small and some profile-adoption combinations are rare.
...
The scatterplot shows a clear upward pattern: states with more EV registrations per resident tend to have more public charging ports per resident. The affluent dense profile occupies the high-adoption, high-infrastructure portion of the space, while lower-income pressured states tend to cluster in lower-adoption and lower-access regions.
...
The conditional probability table suggests that adoption and context compound each other. High EV adoption is associated with high charging access in both mixed sparse and affluent dense profiles, but the highest estimated probability occurs when high adoption appears inside the affluent dense profile. Conversely, low-adoption states in the lower-income pressured profile have very low probability of high public charging access.

### Predictive Evaluation

I evaluate the network using leave-one-out prediction of the discretized charging access level. For each held-out state, the conditional probability tables are re-estimated from the remaining states, and the model predicts the charging access level using the held-out state's context profile and EV adoption level.
